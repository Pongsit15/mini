...
if (isset($_REQUEST['btn_update'])) {
    $Shelf_up = $_REQUEST['txt_Shelf'];
    $product_name_up = $_REQUEST['txt_product_name'];
    $C_Color_up = $_REQUEST['txt_C_Color'];
    $T_Type_up = $_REQUEST['txt_T_Type'];
    $S_Size_up = $_REQUEST['txt_S_Size'];
    $W_Weight_up = $_REQUEST['txt_W_Weight'];
    $Price_up = $_REQUEST['txt_Price'];
    if (empty($Shelf_up)) {
        $errorMsg = "Please Enter Fisrtname";
    } else if (empty($product_name_up)) {
        $errorMsg = "Please Enter Lastname";
    } else if (empty($C_Color_up)) {
        $errorMsg = "Please Enter email";
    } else if (empty($T_Type_up)) {
    $errorMsg = "Please Enter position";
} else if (empty($S_Size_up)) {
    $errorMsg = "Please Enter password";
} else if (empty($W_Weight_up)) {
    ...
    $errorMsg = "Please Enter urole";
        ...
        try {
            if (!isset($errorMsg)) {
                $update_stmt = $db->prepare("UPDATE db_product SET Shelf = :Shelf_up, product_name = :product_name_up, C_Color = :C_Color_up, T_Type = :T_Type_up , S_Size = :S_Size_up, W_Weight = :W_Weight_up, Price = :Price_up  WHERE id = :id");
                $update_stmt->bindParam(':Shelf_up', $Shelf_up);
                $update_stmt->bindParam(':product_name_up', $product_name_up);
                $update_stmt->bindParam(':C_Color_up', $C_Color_up);
                $update_stmt->bindParam(':T_Type_up', $T_Type_up);
                $update_stmt->bindParam(':S_Size_up', $S_Size_up);
                $update_stmt->bindParam(':W_Weight_up', $W_Weight_up);
                $update_stmt->bindParam(':Price_up', $Price_up);
                $update_stmt->bindParam(':id', $id);
                if ($update_stmt->execute()) {
                    $updateMsg = "กำลังดำเนินการเเก้ไขข้อมูลสินค้า...";
    ...
    <section id="content">
       ...
       <div class="left">
           ...
           <ul class="breadcrumb">
               ...
               <li>
                   <a class="active" href="#">เเก้ไขข้อมูลสินค้า</a>
               </li>
           </ul>
       </div>
   </div>
                            ...
                            <div class="form-group text-center">
                                <div class="row">
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="form-group text-center">
                                            <div class="row">
                                                <label for="Price" class="col-sm-1 control-label">ราคาสินค้า</label>
                                                <div class="col-sm-4">
